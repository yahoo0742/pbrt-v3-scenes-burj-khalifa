...
int makesky(int argc, char *argv[]) {
    ...
    for (i = 0; i < argc; ++i) {
        if (!strcmp(argv[i], "--outfile") || !strcmp(argv[i], "-outfile")) {
            ...
            } else if (std::get<0>(arg) == "afternoon") {
                afternoon = std::get<1>(arg) !=0;
            } else
                usage();
        }
    }
    ...
    ParallelFor([&](int64_t t) {
        ...
        for (int p = 0; p < nPhi; ++p) {
            ...
            if (afternoon) {
                phi = 2 * Pi - phi; // With this calculation, the sky light texture is only for the location around the equator.
            }
